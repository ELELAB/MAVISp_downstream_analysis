...
def apply_cat_feature(series: pd.Series, spec: CatSpec) -> pd.Series:
    # Build mapping:
    # spec.order might look like:
    #   ["Destabilizing,Stabilizing", "Uncertain", "Neutral"]
    # We want Destabilizing and Stabilizing to share the same top score.

    # number of tiers
    n_tiers = max(len(spec.order) - 1, 1)

    scores: Dict[str, float] = {}

    for tier_index, tier_raw in enumerate(spec.order):
        # tier_raw might be "Destabilizing,Stabilizing"
        tier_values = [t.strip() for t in tier_raw.split(",") if t.strip()]

        # score goes 1.0 -> 0.0 from first tier to last tier
        if n_tiers > 0:
            score_val = 1.0 - (tier_index / n_tiers)
        else:
            score_val = 1.0  # single-tier edge case

        for tv in tier_values:
            scores[tv.casefold()] = score_val
