...
def clean_feature_label(label: str) -> str:
    """
    Remove '[md]' only for:
      1) Local Int. / Local Int. With DNA feature labels
      2) 'is site part of phospho-SLiM [md]'
    Leave everything else untouched.
    """
    # Case 1: Local interaction features (incl. DNA)
    if label.startswith("Local Int."):
        return label.replace("[md]", "").replace("  ", " ").strip()

    # Case 2: phospho-SLiM site annotation
    if label == "is site part of phospho-SLiM [md]":
        return "is site part of phospho-SLiM"
...
def main():
    ...
    ax.set_xticklabels(
    [clean_feature_label(disp) for disp, _ in feat_cols],
    rotation=45,
    ha="right",
    fontsize=10,
)
    ax.set_title(args.title, pad=14, fontsize=12)
    ...
    # Save (raster + editable PDF)
    out_path = Path(args.output)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    # Save original format (png/svg/etc.)
    fig.savefig(out_path, dpi=args.dpi, bbox_inches="tight")

    # Also save editable PDF
    ...
    print(f"[INFO] Saved editable PDF to: {pdf_path}")
